...
<div class="container-fluid pt-4 px-4">
  <div class="row g-4">
    <div class="bg-light rounded h-100 p-4">
      <form action="{{route('about.card.offer.editpost')}}" method="POST" enctype="multipart/form-data">
        @csrf
        @if(Session::has('msg'))
        ...
        @endif
        <h6 class="mb-4">Card Offer Add Post</h6>
        <div class="form-floating mb-3">
          <p>Card Text</label>
          <textarea name="cardText" class="form-control mb-3" placeholder="Card Image"
              id="floatingTextarea" style="height: 150px;">{{$cardOffer->cardText}}</textarea>
              @error('cardText')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror
        </div>
        <div class="form-floating mb-3">
          <p>Card Text EN</label>
          <textarea name="cardText_en" class="form-control mb-3" placeholder="Card Image"
              id="floatingTextarea" style="height: 150px;">{{$cardOffer->cardText_en}}</textarea>
              @error('cardText_en')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror
        </div>
        <div class="form-floating mb-3">
          <p>Card Text RU</label>
          <textarea name="cardText_ru" class="form-control mb-3" placeholder="Card Image"
              id="floatingTextarea" style="height: 150px;">{{$cardOffer->cardText_ru}}</textarea>
              @error('cardText_ru')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror
        </div>
        <div class="form-floating mb-3">
          <p>Card Text UZ</label>
          <textarea name="cardText_uz" class="form-control mb-3" placeholder="Card Image"
              id="floatingTextarea" style="height: 150px;">{{$cardOffer->cardText_uz}}</textarea>
              @error('cardText_uz')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror
        </div>
        <input type="hidden" name="cardOfferID" value="{{$cardOffer->id}}">
